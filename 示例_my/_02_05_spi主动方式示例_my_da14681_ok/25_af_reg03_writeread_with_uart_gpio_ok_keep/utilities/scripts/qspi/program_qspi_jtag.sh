...
cleanup() {
	...
	grep "${JLINK_PATH:+${JLINK_PATH}/}$JLINKGDB" /proc/$GDBPID/cmdline >/dev/null 2>&1
	if [ $? -eq 0 ] ; then
		kill $GDBPID
	fi
}
...
if [ $# -eq 1 ] ; then
    ...
    JLINKGDB=${JLINK_PATH:+${JLINK_PATH}/}$JLINKGDB
else
    echo "Usage $0 <image file to burn> [JLink path]"
    exit 1
fi
...
if [ -z "${CFG}" ] ; then
  ...
  "$base_dir/prepare_local_ini_file.sh" --cfg "${CFG}" --id $DEVICE_ID ${JLINK_PATH:+--jlink_path ${JLINK_PATH}}
fi
