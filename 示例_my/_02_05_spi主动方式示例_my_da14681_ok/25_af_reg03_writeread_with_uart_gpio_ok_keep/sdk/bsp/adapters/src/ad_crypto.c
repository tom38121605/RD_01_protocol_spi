...
_AD_CRYPTO_ECC_RAM_ATTR static uint32_t ad_crypto_shared_ecc_mem[AD_CRYPTO_SHARED_ECC_RAM_SIZE/4] __attribute__((aligned(1024)));
...
void ad_crypto_init(void *params)
{
...
#if dg_configUSE_HW_AES_HASH
        OS_ASSERT(_AD_CRYPTO_INIT_AES_HASH_LOCK(aes_hash_lock) == OS_MUTEX_CREATE_SUCCESS);
        ...
        OS_ASSERT(_AD_CRYPTO_INIT_ECC_LOCK(ecc_lock) == OS_MUTEX_CREATE_SUCCESS);
