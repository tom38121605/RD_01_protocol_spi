...
static int16_t socf_vbat_via_adapter(void)
{
        ...
        voltage = ad_battery_raw_to_mvolt(src, ad_battery_read(src));
        ...
        const int16_t *v_high)
{
        ...
        } else if (-current > SOCF_BATT_HIGH_CURRENT) {
                return (socf_chk_lut(v_high, socf_ref, now_voltage, VOL2SOC_LUT_SIZE));
        } else {
...
void socf_full_charged_notification()
{
        ...
        if (socf_vbat_via_adapter() > vol_dis_low[VOL2SOC_LUT_SIZE - 1]) {
