...
{
        irb_ble_register_cmd_t *cmd;
        irb_ble_register_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* setup IRB with new message and fill it */
        cmd = alloc_ble_msg(IRB_BLE_REGISTER_CMD, sizeof(*cmd));
        cmd->task = OS_GET_CURRENT_TASK();

        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_register_cmd)) {
...
ble_error_t ble_enable(void)
{
        ...
        /* setup IRB with new message and fill it */
        cmd = alloc_ble_msg(IRB_BLE_ENABLE_CMD, sizeof(*cmd));

        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_enable_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
{
        irb_ble_reset_cmd_t *cmd;
        irb_ble_reset_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* setup IRB with new message and fill it */
        cmd = alloc_ble_msg(IRB_BLE_RESET_CMD, sizeof(*cmd));

        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_reset_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_evt_hdr_t *ble_get_event(bool wait)
{
        ...
        ret = ble_mgr_event_queue_get(&irb, wait ? OS_QUEUE_FOREVER : OS_QUEUE_NO_WAIT);
        ...
        // remove ptr from IRB so it's not freed (should be freed by application)
        irb.ptr_buf = NULL;
...
bool ble_has_event(void)
{
        ...
        return uxQueueMessagesWaiting(mgr_if->evt_q) > 0;
}

ble_error_t ble_read_tx_power(uint16_t conn_idx, tx_power_level_type_t type, uint8_t *tx_power)
{
        irb_ble_read_tx_power_cmd_t *cmd;
        irb_ble_read_tx_power_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* setup IRB with new message and fill it */
        cmd = alloc_ble_msg(IRB_BLE_READ_TX_POWER_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->type = type;

        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_read_tx_power)) {
                return BLE_ERROR_FAILED;
        }
