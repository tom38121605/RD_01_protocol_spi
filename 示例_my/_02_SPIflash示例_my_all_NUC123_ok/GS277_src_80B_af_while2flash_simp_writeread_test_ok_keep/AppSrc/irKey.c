...
u32 ir_key_bits = 0;	// store bits when ir decode  
static u32 ir_key_bit_mask = 0; 	 // mask which indicate the  bit that ir pulse corresponded
enum KEYWORD last_key = NONE_KEY;

/*****add  ir_key_scan_flag ****/
// u8 ir_key_scan_flag = 0;      //ma 20150702  

/****ir key volue tese***/
u8 ir_key_volue = 0;    //ma 20150618   

u8 ir_key_flag = 0;        //ma 20150728  add ir_key flag 
...
// static u32 ir_key_bits_pool[8], ir_key_bits_index = 0;     //ma 20150728  del useless array ir_key_bits_pool
static u32 ir_key_bits_index = 0;      //ma 20150728  after del 
...
do{ // ir debug
	...
	if(ir_pulse>IR_BIT_PULSE_MAX){	// is not bit signal may be leader code or            //MA 20150720  24
		if(ir_state != IR_LEADER_STATE)
			switch(ir_pulse/10){
				...
				case IR_REPEAT_1_HEAD_CODE:	case IR_REPEAT_1_HEAD_CODE+1: // the first reqeat code
...
end1:				if(ir_pulse > 960){	// bigger than the biggest repeated header code pulse time
		...
		switch(ir_pulse){
			...
			case IR_BIT_0_CODE:                           //MA 20150720  11 
					...
					if(ir_key_bit_mask == 0){
						ir_state = IR_IDLE_STATE;
#if(IR_DEBUG_EN)
// 						ir_key_bits_pool[ir_key_bits_index++] = ir_key_bits;   //ma 20150728  del useless array ir_key_bits_pool
						ir_key_bits_index++;              //ma 20150728   add after del function
						ir_key_bits_index	&= 7;
#endif
			...
			case IR_BIT_1_CODE:                                             //MA 20150720  22
...
// 						ir_key_bits_pool[ir_key_bits_index++] = ir_key_bits;              //ma 20150728  del useless array ir_key_bits_pool
			...
			case IR_BIT_01_000_CODE:                                     //MA 20150720  33 
			...
			case IR_BIT_11_100_CODE:                                       //MA 20150720  44 
...
#endif
// 		ir_key_volue = (u8)ir_decode.byte2; //ma 20150618 
// 		play_music_flag =17;    //ma 20150618 
// 		key_time_count  =0;       //ma 20150617
		ir_key_flag = 1;              //ma 20150728		
		switch(ir_decode.byte2){
			...
			case IR_K4:
				return DEC_KEY;
			case IR_K5:
				return MULTIROOM_KEY;
			case IR_K6:
				return SOUND_CLOSE_KEY;
// 			case IR_K7:
// 				return FACTORY_KEY_1;
// 			case IR_K8:
// 				return FACTORY_KEY_2;
// 			case IR_K9:
// 				return FACTORY_KEY_3;
// 			case IR_K10:
// 				return FACTORY_KEY_4;
			case IR_K11:
				return FACTORY_KEY_1;
			case IR_K12:
				return FACTORY_KEY_2;
...
// 				return FACTORY_KEY_4;
			...
			/*******remap lab ir key****/
			case LAB_BT_MODE_KEY:
				return LAB_BT_MODE;
			
			case LAB_BT_PAIRING_MODE_KEY:
				return LAB_BT_PAIRING_MODE;
			case LAB_OPTICAL_MODE_KEY:
				return LAB_OPTICAL_MODE;
			case LAB_LINE_IN_MODE_KEY:
				return LAB_LINE_IN_MODE;
			case LAB_NET_WORK_RX_MODE_KEY:
				return LAB_NET_WORK_RX_MODE;
			case LAB_VOL_MAX_KEY:
				return LAB_VOL_MAX;
			case LAB_SPEAK_1_KEY:
				return LAB_SPEAK_1;
			case LAB_SPEAK_2_KEY:
				return LAB_SPEAK_2;
			case LAB_SPEAK_3_KEY:
				return LAB_SPEAK_3;
			case LAB_SPEAK_4_KEY:
				return LAB_SPEAK_4;
			case LAB_SPEAK_5_KEY:
				return LAB_SPEAK_5;
			case LAB_SPEAK_ALL_KEY:
				return LAB_SPEAK_ALL;
 ...
 * @details: 	 do test key
 *-----------------------------------------------------------------------*/
// void devTestDealIrKey(enum KEYWORD ir_keyword)      //ma 20150707   del useless function   
// {
...
// 			devTestDealIrKey(irkey_wrod);	// if dev is in DEV_TEST STATE do it
			break;
		case DEV_STANDBY:
// 			ir_key_scan_flag = 1;      //ma 20150702   set ir_key_scan_flag 
