...
uint32_t CLK_GetAPBClockFreq()
{
	return CLK_GetHCLKFreq() / (CLK->APBDIV + 1);
}
/**
...
void DrvI2C_Init(I2C_T *I2cPort, uint32_t u32BusClock, uint8_t EnableInt)
{
    ...
    divider = (uint32_t) (((CLK_GetAPBClockFreq()*10)/(u32BusClock * 4) + 5) / 10 - 1);	/* Compute proper divider for I2C clock */
