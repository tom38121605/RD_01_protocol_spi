...
u32 daL=0, daR=0;
volatile static u32 act_interval = 0;
GsData gsdata;
s16 rawData[4];
s16 hpfData[4];
s16 lpfData[4];
...
static void judgePeaks()
{
	...
	for(i = 0; i < 4; i++){
		...
		if(	abs(gs_channels[i].min_frame - gs_channels[i].max_frame) < 40
			&& gs_channels[i].max - gs_channels[i].min > (THR_PEAK<<1)
	...
	if(flag){
		if(abs(last_counter - gs_channels[0].max_frame) > 40){
...
{	
	if(abs(frame_counter - gs_channels[channel].max_frame) > 50 
...
{
	if(abs(frame_counter - gs_channels[channel].min_frame) > 50  
...
static void gsResetDone(void)
{
	...
	gs_i2c_data.buf[0] = 0x2f;
	gs_i2c_data.buf[1] = 0x01;
	gs_i2c_data.buf[2] = 0x20;
	gs_i2c_data.buf[3] = 0x06;
...
error:			sendInfo("i2c read gs error!\r\n");
				...
				for(i=0; i<8; i++){
					gs_i2c_data.buf[i] = gs_i2c_data.buf[1+i];
				}
				if(gs_verify())
					goto error;
				...
				/*
				uartdecode(lpfData[0], ":");
				uartdecode(lpfData[1], ":");
				uartdecode(lpfData[2], ":");
				uartdecode(lpfData[3], ":");
...
void do_gs_event(void)
{
	...
	analyseMaxFrame((u32 *)ch_max);
	analyseMinFrame((u32 *)ch_min);
